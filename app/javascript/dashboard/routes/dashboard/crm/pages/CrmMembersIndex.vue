...
const L = {
  ...
  error: '保存失败',
  loading: '加载中…',
  hint: '权限口径：超级管理员=一切；管理员=一切，除了任免/修改超级管理员；部门负责人=自己部门的一切（数据、考核打分、下属密码、考勤汇总）；业务员=只管自己；无角色=仅共享模块（文档/审批/沟通/考勤）。行政部门成员额外可维护审批模板并自改密码。模块开关控制能否使用对应业务系统（关闭 CRM 后即使有角色也无法进入）。',
};
