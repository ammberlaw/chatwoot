...
const canManage = computed(() => {
  ...
  return role === 'administrator' || currentUser.value?.crm_role === 'manager';
});
      ...
      <table v-else class="w-full text-sm text-left border-collapse">
        <thead class="text-n-slate-11">
          <tr class="border-b border-n-weak">
            <th class="px-3 py-2 font-medium">{{ t('CRM.KPI_SCHEMES.TABLE.NAME') }}</th>
            <th class="px-3 py-2 font-medium">{{ t('CRM.KPI_SCHEMES.TABLE.MONTH') }}</th>
            <th class="px-3 py-2 font-medium">{{ t('CRM.KPI_SCHEMES.TABLE.STATUS') }}</th>
            <th class="px-3 py-2 font-medium">{{ t('CRM.KPI_SCHEMES.TABLE.PASS_SCORE') }}</th>
            <th class="px-3 py-2 font-medium">{{ t('CRM.KPI_SCHEMES.TABLE.ITEMS') }}</th>
            <th class="px-3 py-2 font-medium">{{ t('CRM.KPI_SCHEMES.TABLE.TIERS') }}</th>
        ...
        <tbody>
          ...
          >
            ...
            <td class="px-3 py-2 text-n-slate-11">{{ fmtMonth(record.schemeMonth) }}</td>
            <td class="px-3 py-2">
              <span
            ...
            <td class="px-3 py-2 text-n-slate-11">{{ record.passScore ?? '—' }}</td>
            <td class="px-3 py-2 text-n-slate-11">{{ (record.schemeItems || []).length }}</td>
            <td class="px-3 py-2 text-n-slate-11">{{ (record.payoutTiers || []).length }}</td>
