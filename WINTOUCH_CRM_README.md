...
**数据范围**：超管/管理员看全部团队；部门负责人与业务员仅看自己所属团队（`teams` 接口同口径收口，筛选下拉不再出现别的团队）。
...
- **筛选**：全部/我的/私海/公海/未分配 + 客户名搜索(`q`) + 客户分组 + 产品分组；分页。**团队/业务员筛选按角色**：超管/管理员=团队→业务员级联（全量）；部门负责人=仅自己团队的业务员下拉；**公海视图无团队/业务员筛选**（全公司共享池）。
- **负责人双视图**：私海页顶部「我的客户 / 团队客户」切换；团队客户=本人+辖区成员的客户。
- **客户分配（勾选批量）**：私海列表勾选行 →「分配给… + 分配」；**任何 CRM 角色都可把自己名下客户分配给任意成员**；负责人分配下属客户时目标限本团队；管理员不受限。分配级联：客户名下**商机（私海）、订单、报价、往来邮件**一并转入新负责人（`POST crm/customers/reassign`，按钮悬停有级联提示）。
...
- **+新建商机**（入口从列表移到看板）。
- **筛选按角色**：超管/管理员=团队→业务员级联；部门负责人=仅自己团队的业务员下拉（不再出现团队级）；商机公海视图无筛选。
...
- **新建订单**：PI号必填（红星标识 + 保存拦截）、附件必填（PI/生产订单）。
- **业务员筛选**：负责人视角只列自己团队成员（订单管理与「未关联客户的订单」共用组件，两处一致）；数据范围按负责人辖区收口。
...
- **负责人联动（仅业务条线）**：`Org::Department` 设/卸 `leader_id` 自动同步 `crm_role`（manager ↔ sales，超管/管理员不动）；**只有业务部门**（自身或上级名称含「销售/业务」）触发联动，生产/人事等部门的负责人不会因此获得 CRM 入口（需要时可在成员权限手动授角色）。
- **组织架构维护** `org_maintainer?`：超管/管理员或人事部门成员（部门名含「人事」，含下级）。**人事部成员两项横切特权**：组织架构维护 + 审批模板维护（自改密码 2026-07-17 起已放开全员，不再是特权）。
- **「待我审批」入口**：仅审批人可见（管理角色天然是；其他成员看是否被审批模板点名或名下有审批步骤，`counts` 接口返回 `approver_view`）；非审批人的 OA 卡片显示「我的申请」。
- **在线状态**：系统按实时连接判定（连着=在线，断开=离线），个人菜单只读展示，`profile/availability`、`profile/auto_offline` 接口已封（403）。
...
- **密码管理**：全员可自改密码（个人资料 + 忘记密码邮件，2026-07-17 起放开；`AccountUser#password_self_service?` 恒为 true，守卫保留便于日后收紧）；超管/管理员可在成员权限改成员姓名/邮箱/重置密码；**部门负责人**限下属、仅重置密码（`manager_overreach?`）。
...
- **选择联系人**：按组织架构分组展示（部门为节，未分部门兜底），成员带职位胶囊；顶部搜索框可按**姓名 / 邮箱 / 职位**过滤（数据来自 `org/departments` + `org/memberships`）。
